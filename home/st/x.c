...
mousereport(XEvent *e)
{
	...
	char buf[40];
	static int ox, oy;
	//int isscrolling = button != Button1 && button != Button2 && button != Button3; // used later, self-explanatory
	...
	if (e->xbutton.type == MotionNotify) {
		...
		if (e->xbutton.type == ButtonPress) {
			...
			if (IS_SET(MODE_MOUSEX10))
				return;
			if (button == 64 || button == 65 ) //|| isscrolling) // added || isscrolling defined above, before all these strange ifs
				return;
		}
	}
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og)
{
	...
	if (cursor_follows_color) { // all until else is rarely unstable
	uint32_t cc;
 	if (selected(cx, cy)) {
		cc = g.bg;
 	} else {
	...
	} else { // after if(cursor_follows_color)
	if (IS_SET(MODE_REVERSE)) {
		g.mode |= ATTR_REVERSE;
		g.bg = defaultfg;
		if (selected(cx, cy)) {
