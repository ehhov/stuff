...
char *ampmstyle[2]={"am","pm"};
const char *net_downup="↓↑";
char *Xkb_group_indicator[2]={"🇺🇸","🇷🇺"};
char *Xkb_group_indicator_text[2]={"us","ru"};
char *brightness_max_indicator[2]={"🔅","🔆"};
char *volume_icons[4]={"🔇","🔈","🔉","🔊"};

char *scale_blocks[31]=
	{"░░░░░░░░░░","▒░░░░░░░░░","▓░░░░░░░░░",
	 "█░░░░░░░░░","█▒░░░░░░░░","█▓░░░░░░░░",
	 "██░░░░░░░░","██▒░░░░░░░","██▓░░░░░░░",
	 "███░░░░░░░","███▒░░░░░░","███▓░░░░░░",
...
Timenow calc_time ();
Network calc_netspeed ();
//int calc_layout (Display *);
...
Volume vol; /* global because of signal capturing */
int kb_layout;
...
{
	/*
	const int net_len=IW_ESSID_MAX_SIZE+20, brt_len=20, vol_len=10, kb_len=4, bat_len=20, time_len=20;
  ...
  sigaction(SIGUSR2, &action, NULL);
	...
	while (1) 
	{
		...
		printf(SP);
		color("666666");
		printf("> %s:%.2lf%s%.2lf "PS, net.essid, (net.in>0)? net.in:0, net_downup, (net.out>0)?net.out:0);
		
		//printf(">%d%s"END, bright.percent, brightness_max_indicator[bright.is_max]);
		printf("%s", brightness_max_indicator[bright.is_max]);
		
		//snprintf(vol_str, vol_len, "%s:%s%ld%s", vol.is_on?"V":"M", vol.percent<10?" ":"", vol.percent, vol.percent<100?"%":"");
		//printf("%s", scale_dots[vol.scale]);
		printf("%s",volume_icons[vol.icon]);
		
		printf(" %s ", Xkb_group_indicator[kb_layout]);
		...
		if (bat.percent>75)
			...
			color("ff5500");
		printf("> %.2lf%%%s %02i:%02i "PS, bat.percent, (bat.is_chr? "+" : ""), bat.hours, bat.mins);
		
		printf(" %s %s %d %d:%02d %s ", WeekDays[now.now.tm_wday], Months[now.now.tm_mon], now.now.tm_mday, now.now.tm_hour, now.now.tm_min, ampmstyle[now.is_pm]);
